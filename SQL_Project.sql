...
select e.department_id, e.first_name, e.last_name, e.job_id, d.department_name from employees e, departments d
...
where e.salary < any (select salary from employees where job_id = 'MK_MAN');
...
select e.first_name, e.last_name,  e.employee_id, e.job_id from employees e
where e.salary > any (select salary from employees where job_id = 'PU_MAN') and e.job_id <> 'PU_MAN';
...
where salary > (select employee_id from employees where employee_id = 163);
...
select e.first_name || ' ' || e.last_name, e.employee_id, j.job_title from employees e, jobs j
join employees e on j.job_id = d.job_id;
--join locations l on d.;
...
select last_name, city from employees;
...
select e.first_name, d.department_id, d.department_name from departments d
join employees e on d.manager_id = e.employee_id
